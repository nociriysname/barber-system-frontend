...
export const useApi = () => {
    ...
    const updateNewsItemImage = async (id: number, imageFile: File) => {
        await sleep(1500); // Simulate upload
        setNews(prev => prev.map(item => item.id === id ? { ...item, imageUrl: URL.createObjectURL(imageFile) } : item));
    };
    ...
    const addBooking = (bookingDetails: Omit<Booking, 'id' | 'status' | 'created_at' | 'updated_at' | 'appointment_time' | 'service' | 'employee' >) => {
        const newBooking: Booking = {
            ...
            service: services.find(s => s.name === bookingDetails.serviceName) || services[0], // Mock
            employee: masters.find(m => m.name === bookingDetails.masterName) || masters[0], // Mock
        };
    ...
    // Master Management
    const addMaster = async (data: { name: string, branchId: number, position: MasterStatus, username?: string, avatarFile?: File | null }) => {
    ...
    const getOccupiedSlots = (selectedMaster: Master | null, selectedDate: Date) => {
        ...
        const seed = selectedMaster.id + selectedDate.getDate();
        const occupied = new Set<string>();
        // Simple mock logic for occupied slots
